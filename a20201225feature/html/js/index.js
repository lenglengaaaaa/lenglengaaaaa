$(function(){
    $('#fullpage').fullpage({
        ...
        afterRender: function(){
            ...
            // 暂时隐藏后面两屏
            // $('.thirdPage').remove();
            // $('.nav,.btn-box,.icon-slide,.notice-box').hide();
...
$('.btn-box a').hover(function(){
    ...
    if (OUT.user.isTime == 0) {
        ulink.alert('即将解锁周杰伦限定大奖，敬请期待');
        return;
    }
    var idx=$(this).index();
    $(this).addClass('on').siblings('a').removeClass('on');
    if(idx==1){
        window.location.href = OUT.site.pcLink;
    }
})
...
var mySwiper = new Swiper('.swiper-img-box .swiper-container', {
    autoplay: {
        delay: 3000,
...
function voiceClick(click){
    ...
    $(click).on('click',function(){
        ...
        $('#voice').attr('src','https://game.gtimg.cn/images/codm/cp/a20201209codm/voice' +(index + 1) + '.wav');
        if(player.paused){
            player.play();
