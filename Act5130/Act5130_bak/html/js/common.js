...
var shareOptions = {
    ...
    imgUrl: 'https://game.gtimg.cn/images/slg/act/5130/a20210225newstar/icon_share.jpg', // 分享后朋友看到的图标，大小不能超过32K
...
function toPage() {
    ...
    var url = 'https://slg.qq.com/act/5130/a20210225newstar/page.html';
    if (ulink.isMSDK()) {
...
function toHome() {
    var url = window.location.href = 'https://slg.qq.com/act/5130/a20210225newstar/index.html';
    if (ulink.isMSDK()) {
        url = url + window.location.search;
    }
    window.location.href = url;
...
// 弹出投注弹框
function showPoulPop(key, type, optionId) {
    console.log(OUT.user.betList[key][0]);
    var tmpData = OUT.user.betList[key][0];
    if (!tmpData) {
        return;
    }

    if (type == 1 && tmpData['fightBetGroupId'] != 0 && tmpData['fightBetGroupId'] != optionId) {
        return;
    }
    ...
    OUT.user.optionId = optionId;
    OUT.user.type = type;
    $('#dialog-bet1 .bet-right em').eq(0).text(OUT.user.coinNum);
    $("#pourInput").val(1);
    $("#betPour>span").removeClass('on');
    $("#betPour>span").eq(0).addClass('on');
...
function createMorePlayHtml(key) {
    ...
    if (tmpData['sumBetOptionId']) {
        ...
        tmp1.find('.btns').find("a[data-option='" + tmpData['sumBetOptionId'] + "']").removeClass('no').addClass('btn-sub-vote');
    } else {
        tmp1.find('.btns').find('a').removeClass('no');
    }

    var tmp2 = $('#morePlayUl li').eq(1);
    if (tmpData['subBetOptionId']) {
        tmp2.find('.btns').find('a').addClass('no');
        tmp2.find('.btns').find("a[data-option='" + tmpData['subBetOptionId'] + "']").removeClass('no').addClass('btn-sub-vote');
    } else {
        tmp2.find('.btns').find('a').removeClass('no');
    }
}
...
function showPending() {
    showTips('竞猜商店兑换于5月2日0:00开放，敬请期待');
    //TGDialogS('dialog-vote')
}

function goHeight(target) {
