...
// 检查登录态
    ...
    createBetHtml: function () {
        for (var gNo = 1; gNo <= 16; gNo++) {
            ...
            for (var k in data) {
                ...
                html += '<p class="competitors-name"><span>' + data[k].league1Name + ' </span>  <span>' + data[k].group1Name + '</span></p>';
                ...
                html += '<p class="competitors-name"><span>' + data[k].league2Name + ' </span>  <span>' + data[k].group2Name + '</span></p>';
                ...
                if (sumNum >= 10000) {
                    var betSumNum = Math.floor(sumNum / 10000);
                    if (betSumNum <= 0) {
                        betSumNum = 1;
                    }
                    sumNum = betSumNum + '万';
    ...
    switchPrizeTip: function () {
        if ($('.btn-dialog-bet-end').hasClass('on')) {
            $("#dialog-active-tip").text('每周一更新竞猜结果数据');
        } else {
            $("#dialog-active-tip").text('每日0：00刷新任务进度');
        }
    },
    // 领取任务奖励
        ...
        ulink.http.get({
            ...
            success: function (result) {
                ...
                if (result.iRet == 4001) {
                    ...
                    closeDialog();
                    $('.btn-dialog-day-act').hasClass('on')
    ...
    getWishData: function () {
        ...
        ulink.http.get({
            ...
            success: function (result) {
                ...
                if (OUT.user.maxWishCoinNum < 0) {
                    OUT.user.maxWishCoinNum = 0;
                }
                $("#wishAllowMax").text(OUT.user.maxWishCoinNum);
                OUT.user.coinNum = parseInt(result.jData.userCoinNum);
                if (result.jData.hasOwnProperty('isEnd')) {
                    console.log('write wish is end');
                    OUT.user.wishIsEnd = result.jData.isEnd;
                }
    ...
    // 许愿
    wishDialog: function () {
        if (OUT.user.wishIsEnd) {
            showTips('许愿池活动已结束~');
            return;
        }
        TGDialogS('dialog-bet');
        $('.btn-dialog-wish-rule').removeClass('on');
        $('.btn-dialog-wish').addClass('on');
        ...
        ulink.http.get({
            ...
            success: function (result) {
                ...
                for (var key in data) {
                    ...
                    var spanHtml = "<span>";
                    var splitDate = data[key].matchTime.split(" ");
                    spanHtml += splitDate[0] + " ";
                    // 时间+竞猜类型+比赛双方
                    if (data[key].betType == 1) {
                        spanHtml += "胜负竞猜 ";
                    } else if (data[key].betType == 2) {
                    ...
                    spanHtml += "<br>" + data[key].team1GroupName + ' vs ' + data[key].team2GroupName + "<br>投注"+data[key].coinNum+"</span>";
                    if (data[key].betStatus == 0) {
                        spanHtml += '<span></span>';
                    } else if (data[key].betStatus == 1) {
                        spanHtml += '<span>可领取' + data[key].prizeNum + '</span>';
                        spanHtml += '<a class="btn-dialog-receive fail" href="javascript:;" onclick="PTTSendClick(\'btn\',\'btn-dialog-receive\',\'领取\');" title="领取"></a>';
                    } else if (data[key].betStatus == 2) {
                        spanHtml += '<span>可领取' + data[key].prizeNum + '</span>';
                        spanHtml += '<a class="btn-dialog-receive" href="javascript:OUT.user.getPrize(' + key + ',' + data[key].betId + ');" onclick="PTTSendClick(\'btn\',\'btn-dialog-receive\',\'领取\');" title="领取"></a>';
                    } else if (data[key].betStatus == 3) {
                        spanHtml += '<span>可领取' + data[key].prizeNum + '</span>';
                        spanHtml += '<a class="btn-dialog-receive on" href="javascript:;" onclick="PTTSendClick(\'btn\',\'btn-dialog-receive\',\'领取\');" title="领取"></a>';
                    }
                    html += spanHtml + "</div>";
                }
                $('.act-cont>.scroll-box').html(html);
