            ...
            success: function (result) {
                ...
                for (var k = 1; k <= 5; k++) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        //  第一赛区-小组赛第一轮
                        var gNo = groupNoList[data[i].matchGroup];
                        var wNo = groupNoList[data[i].weekNo];
                        sHtml += '<li><p class="time">' + data[i].matchDay + ' ' + "第" + gNo + "赛区" + '-小组赛第'+wNo+'轮</p><div class="match-item">';
                        if (data[i].winnerGroupId == data[i].team1Id) {
