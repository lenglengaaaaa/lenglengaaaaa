var VSHAREKEY = htmlEncodeByRegExp(milo.request('vShareKey'));
var e_code = 511552;
var TRACEINFO='';
var NICKNAME='';
var PAGE=0;
...
amsCfg_684797 = {
    ...
    "fFlowSubmitEnd": function(res){
        ...
        var endTime="2020/08/23 23:59:59";
        var nowTime=date('Y/m/d H:i:s', strtotime(getSeverDateTime()));
        if(nowTime>endTime){
            $("#whMsg").html('(维护期间暂停兑换)');
        }else{
            $("#whMsg").html('(动态刷新，详情查看活动规则)');
        }
        //console.log(weekLimitArr)
        //维护显示
        if(nowTime>endTime){
            weekLimitArr=["0", "0", "0", "0"];
        }
...
amsCfg_682068 = {
    ...
    "fFlowSubmitEnd": function(res){
        if(res.jData.result=='0'||res.jData.result=='2'){
            var nickName=htmlEncodeByRegExp(decodeURIComponent(decodeURIComponent(milo.request('nick'))));
...
function htmlEncodeByRegExp(str){
  ...
  s = s.replace(/\"/g,"&quot;");
  return s;
}/* #t6Hl8#FF07D2198159C6B5C8F7734261D3B409 */